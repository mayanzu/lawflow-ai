...
export default function ConfirmPage() {
  ...
  const [info, setInfo] = useState<Info>({ 案号:'', 案由:'', 原告:'', 被告:'', 判决法院:'', 判决结果:'', 上诉期限:'', 上诉法院:'', 判决日期:'' })
  ...
  function parseDate(s: string): string | null {
    ...
    if (m1) { let y = parseInt(m1[1]), mo = parseInt(m1[2]), d = parseInt(m1[3]); if (y < 100) y += 2000; if (y > 1990 && y < 2031 && mo >= 1 && mo <= 12 && d >= 1 && d <= 31) return `${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}` }
    const m2 = s.match(/([零〇一二三四五六七八九十]+)年([零〇一二三四五六七八九十]+)月([零〇一二三四五六七八九十]+)日/)
    if (m2) { let y = cnToInt(m2[1]), mo = cnToInt(m2[2]), d = cnToInt(m2[3]); if (y < 100) y += 2000; if (y > 1990 && y < 2031 && mo >= 1 && mo <= 12 && d >= 1 && d <= 31) return `${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}` }
    const m3 = s.match(/(\d{4})年([零〇一二三四五六七八九十]+)月([零〇一二三四五六七八九十]+)日/)
    if (m3) { const y = parseInt(m3[1]), mo = cnToInt(m3[2]), d = cnToInt(m3[3]); if (mo >= 1 && mo <= 12 && d >= 1 && d <= 31) return `${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}` }
    return null
  }

  function extractDatesFromOcr(ocrText: string): string | null {
    const allDates: string[] = []
    for (const m of ocrText.matchAll(/(\d{2,4})年(\d{1,2})月(\d{1,2})日/g)) { let y = parseInt(m[1]), mo = parseInt(m[2]), d = parseInt(m[3]); if (y < 100) y += 2000; if (y > 1990 && y < 2031 && mo >= 1 && mo <= 12 && d >= 1 && d <= 31) allDates.push(`${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}`) }
    for (const m of ocrText.matchAll(/([零〇一二三四五六七八九十]+)年([零〇一二三四五六七八九十]+)月([零〇一二三四五六七八九十]+)日/g)) { let y = cnToInt(m[1]), mo = cnToInt(m[2]), d = cnToInt(m[3]); if (y < 100) y += 2000; if (y > 1990 && y < 2031 && mo >= 1 && mo <= 12 && d >= 1 && d <= 31) allDates.push(`${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}`) }
  ...
  function calcDeadline(判决日期: string, 上诉期限: string) {
    ...
    const deadline = new Date(判决日期); deadline.setDate(deadline.getDate() + days)
  ...
  function handleFieldChange(field: keyof Info, value: string) {
    setInfo(prev => { const next = { ...prev, [field]: value }; if (field === '判决日期' || field === '上诉期限') calcDeadline(field === '判决日期' ? value : prev.判决日期, field === '上诉期限' ? value : prev.上诉期限); return next })
  }
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const res = await fetch('/api/generate-appeal', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ info, ocr_text: ocrText }) })
      const data = await res.json()
      if (data.success) {
        localStorage.setItem('lw_appeal_text', data.appeal || '')
        if (data.legal_basis?.length > 0) localStorage.setItem('lw_legal_basis', JSON.stringify(data.legal_basis))
        router.push('/result')
      } else { alert('生成失败: ' + (data.error || '未知错误')); setGenerating(false) }
    } catch (err: any) { alert('生成失败: ' + err.message); setGenerating(false) }
  }
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        const parsedInfo: Info = { 案号:parsed.案号||'', 案由:parsed.案由||'', 原告:parsed.原告||'', 被告:parsed.被告||'', 判决法院:parsed.判决法院||'', 判决结果:parsed.判决结果||'', 上诉期限:parsed.上诉期限||'', 上诉法院:parsed.上诉法院||'', 判决日期:parsed.判决日期||'' }
        setInfo(parsedInfo)
        if (parsed.上诉法院) {
          // already have it
        } else if (parsed.判决法院) {
          ...
          if (appealCourt) parsedInfo.上诉法院 = appealCourt
        }
        calcDeadline(parsedInfo.判决日期, parsedInfo.上诉期限 || '15')
      } catch {}
    }
    if (ocr && !info.判决日期) {
      const dateStr = extractDatesFromOcr(ocr)
      if (dateStr) { const d = new Date(dateStr); if (!isNaN(d.getTime()) && d.getFullYear() >= 2000 && d.getFullYear() <= 2030) { handleFieldChange('判决日期', dateStr) } }
    }
  ...
  const labelStyle: React.CSSProperties = { fontSize: 12, fontWeight: 600, color: '#5F6368', display: 'block', marginBottom: 6 }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F9FA', fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Text", "Helvetica Neue", sans-serif' }}>
      ...
      <div style={{ padding: '12px 16px', background: '#FFF', borderBottom: '1px solid #E8EAED', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <button onClick={() => router.push('/flow')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', fontWeight: 600, color: '#1D1D1F', padding: '8px 0' }}>← 返回</button>
      ...
      {daysLeft !== null && (
        <div style={{ margin: '16px 16px 0', padding: '12px 16px', borderRadius: 10, background: isExpired ? '#FEF0EF' : isUrgent ? '#FFF3E0' : '#E8F5E9', border: `1px solid ${isExpired ? '#F5C6C5' : isUrgent ? '#FFE0B2' : '#C8E6C9'}` }}>
          <div style={{ fontSize: '13px', fontWeight: 600, color: isExpired ? '#D93025' : isUrgent ? '#E65100' : '#2E7D32', marginBottom: 2 }}>
            {isExpired ? '上诉期已届满' : `上诉期限：剩余 ${daysLeft} 天`}
          </div>
          <div style={{ fontSize: '11px', color: '#5F6368' }}>判决日期：{info.判决日期 || '未填写'} &nbsp;|&nbsp; 届满：{appealDeadline ? new Date(appealDeadline).toLocaleDateString('zh-CN') : '—'}</div>
        </div>
      )}

      <div style={{ padding: '16px 16px 80px', maxWidth: '600px', margin: '0 auto' }}>
        {/* 基本信息 */}
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => router.push('/flow')} style={{ flex: 1, padding: '14px', background: '#FFF', color: '#1D1D1F', border: '1px solid #E0E0E0', borderRadius: 12, cursor: 'pointer', fontSize: '15px', fontWeight: 500, minHeight: 48 }}>返回修改</button>
          <button onClick={handleConfirm} disabled={generating} style={{ flex: 2, padding: '14px', background: '#0071E3', color: '#FFF', border: 'none', borderRadius: 12, cursor: 'pointer', fontSize: '15px', fontWeight: 600, minHeight: 48, opacity: generating ? 0.6 : 1 }}>
            {generating ? '正在生成...' : '确认并生成上诉状 →'}
          </button>
        </div>
      </div>
    </div>
