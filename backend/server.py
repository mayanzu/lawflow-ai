...
def _pdf_to_long_image(path, dpi=150):
    ...
    for p in pages:
        p.close()
    print(f"[long-img] {total} pages -> {max_w}x{total_h}", flush=True)
    return tmp
...
def _ocr_pdf(path):
    ...
    # Step 2: 长图 -> 腾讯云 OCR 一次调用
    if USE_TENCENT_OCR:
        try:
            long_img = _pdf_to_long_image(path, dpi=150)
            try:
                with open(long_img, 'rb') as f:
                    img_b64 = __import__('base64').b64encode(f.read()).decode('utf-8')
                result = _tencent_ocr_base64(img_b64)
                if result and len(result) > 100:
                    print(f"  [long-img] Tencent {len(result)} chars", flush=True)
                    os.remove(long_img)
                    return result
                else:
                    print(f"  [long-img] thin result, falling back", flush=True)
            finally:
                if os.path.exists(long_img):
                    os.remove(long_img)
        except Exception as e:
            print(f"  [long-img] failed: {e}", flush=True)

    # Step 3: RapidOCR 逐页兜底
    ...
    for pg in range(total_pages):
        try:
            ...
            del pix
            res = ocr_local(tmp)
            if res and hasattr(res, 'txts'):
                txts = [t for t in res.txts if t and t.strip()]
                if txts:
                    all_text.append("\n".join(txts))
                    print(f"  P{pg+1}: Local OCR {len(txts)} items", flush=True)
            os.remove(tmp)
        except Exception as e:
...
【2 当事人】
上诉人（原审原告/原审被告）：（从判决书原文提取真实姓名）
被上诉人（原审被告/原审原告）：（从判决书原文提取真实姓名）
...
## 法定格式要求（依据民诉法第165条、最高法院文书样式，违反任何一项都是废纸）
1. 标题：民事上诉状（单独一行）
2. 当事人信息：上诉人/被上诉人的姓名、性别、出生年月、民族、住址、身份证号
3. 上诉请求：必须具体、量化（写明撤销/改判原审判决第几项）
4. 事实与理由：至少分两点，分别针对原审的事实认定错误和法律适用错误展开
5. 结尾格式：
   此致
   [二审法院全称]
   上诉人：[上诉人姓名]
...
【第二部分：当事人信息】
上诉人（原审原告/原审被告）：（从判决书原文提取真实姓名），……（身份证号等身份信息）
被上诉人（原审被告/原审原告）：（从判决书原文提取真实姓名），……
